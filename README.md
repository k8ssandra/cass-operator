...
``cass-operator`` can be used as standalone product to manage your Cassandra cluster or as part of the [k8ssandra-operator](https://github.com/k8ssandra/k8ssandra-operator/). With tooling such as managed repairs and automated backups, see k8ssandra-operator as the recommended approach.
...
```console
kubectl apply --force-conflicts --server-side -k github.com/k8ssandra/cass-operator/config/deployments/default?ref=v1.31.0
```
...
```console
kubectl apply --force-conflicts --server-side -k github.com/k8ssandra/cass-operator/config/deployments/cluster?ref=v1.31.0
```
...
```console
kubectl apply -k github.com/k8ssandra/cass-operator/config/prometheus?ref=v1.31.0
```
...
resources:
  - github.com/k8ssandra/cass-operator/config/deployments/default?ref=v1.31.0

components:
  - github.com/k8ssandra/cass-operator/config/components/cluster?ref=v1.31.0
```
...
cass-operator's default image name patterns and repositories are defined in a [image_config.yaml](https://github.com/k8ssandra/cass-operator/blob/master/config/imageconfig/image_config.yaml) file. The image_config.yaml will define only the deployed Cassandra / DSE and server-system-logger images, not the cass-operator itself. To modify imageConfig when deploying, we can create a kustomize component to replace this file in the deployment with our own.
...
metadata:
  name: image-config
images:
  system-logger: "k8ssandra/system-logger:v1.31.0"
...
```
kubectl apply -f https://raw.githubusercontent.com/k8ssandra/cass-operator/v1.31.0/operator/k8s-flavors/gke/storage.yaml
```
