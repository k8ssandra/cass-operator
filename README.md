...
```console
kubectl apply --force-conflicts --server-side -k github.com/k8ssandra/cass-operator/config/deployments/default?ref=v1.23.0
```
...
```console
kubectl apply --force-conflicts --server-side -k github.com/k8ssandra/cass-operator/config/deployments/cluster?ref=v1.23.0
```
...
```console
kubectl apply -k github.com/k8ssandra/cass-operator/config/prometheus?ref=v1.23.0
```
...
components:
  - github.com/k8ssandra/cass-operator/config/components/cluster?ref=v1.23.0
```
...
metadata:
  name: image-config
images:
  system-logger: "k8ssandra/system-logger:v1.23.0"
...
```
kubectl apply -f https://raw.githubusercontent.com/k8ssandra/cass-operator/v1.23.0/operator/k8s-flavors/gke/storage.yaml
```
