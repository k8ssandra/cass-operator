...
func TestCassandraOverride(t *testing.T) {
	...
	assert.Equal("us-docker.pkg.dev/k8ssandra/images/cass-management-api:4.0.0", cassImage)
...
func TestDefaultImageConfigParsing(t *testing.T) {
	...
	assert.True(strings.HasPrefix(GetImageConfig().Images.SystemLogger, "us-docker.pkg.dev/k8ssandra/images/system-logger:"))
	assert.True(strings.HasPrefix(GetImageConfig().Images.ConfigBuilder, "datastax/cass-config-builder:"))

	assert.Equal("us-docker.pkg.dev/k8ssandra/images/cass-management-api", GetImageConfig().DefaultImages.CassandraImageComponent.Repository)
...
func TestDefaultRepositories(t *testing.T) {
	...
	assert.NoError(err)
	assert.Equal("us-docker.pkg.dev/k8ssandra/images/cass-management-api:4.0.1", path)
